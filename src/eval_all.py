...
# -- main program --
ocr_result_df = pd.DataFrame(columns=["OCR Engine", "Model", "Precision", "Recall", "F1", "Jaccard"])
for ocr_engine in OCR:
    for params in OCR_PARAMS:
        ...
        param_result_df = pd.DataFrame(columns=["Precision", "Recall", "F1", "Jaccard"])
        for file_name in GROUND_TRUTH_PATHS.keys():
            # create normalized dataframe out of csv
            df_gt = normalize_df(pd.read_csv(GROUND_TRUTH_PATHS[file_name]))
            df_output = normalize_df(pd.read_csv(output_paths[file_name])) 
            # run evaluation (Precision, Recall, F1, Jaccard)
            results = create_evaluation(df_gt, df_output)
            param_result_df = pd.concat([param_result_df, pd.DataFrame([results])], ignore_index=True)
        
        # calculate mean values
        mean_results = {
            "OCR Engine": ocr_engine,
            "Model": f"Qwen3:{params}",
            "Precision": param_result_df["Precision"].mean(),
            "Recall": param_result_df["Recall"].mean(),
            "F1": param_result_df["F1"].mean(),
            "Jaccard": param_result_df["Jaccard"].mean()
        }
        ocr_result_df = pd.concat([ocr_result_df, pd.DataFrame([mean_results])], ignore_index=True)
print(ocr_result_df)
print("="*60)

vlm_result_df = pd.DataFrame(columns=["Model", "Precision", "Recall", "F1", "Jaccard"])
for vlm_name, vlm_dir_name in VLM.items():
    ...
    vlm_result_df = pd.concat([vlm_result_df, pd.DataFrame([mean_results])], ignore_index=True)
print(vlm_result_df)
print("="*60)
