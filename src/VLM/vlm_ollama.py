...
def extract_courses(image, vlm_model, context_size) -> pd.DataFrame:
    response: GenerateResponse = generate(
        ...
        prompt=f"""
                # Goal
                You will be given an image of a document likely to be a students transcript of completed courses eventually with additional information like earned credits or accomplished grades for each course.
                Your goal is to figure out whether the image is such a students transcript and in case that it is extract each course name whose field of study falls within either Computer Science or Mathematics.
                ...
                Each `Course(BaseModel)` has four fields:
                    ...
                    3. grade: The grade the student acquired for each Course, only if given in the image. Can be a string if its a letter like "A" or "BB+" or a numerical value like a score or grade "15", "2.3". If not given default to the python value `None`. 
                ...
                ## Example-Outputs
                Following are a couple example JSON-outputs following the schema. Be aware that those are to show different cases and their outputs. Only use values which are in the actually given image for your output.:
                    1. Two examples where all informations where given in the image:
                    ...
                    2. Example where no credits where mentioned in the image:
                ...
                - After classification and extraction, return a JSON that follows the given Pydantic Schema. Do not add any comments before, inbetween or after. If the image you got is no student transcript of any kind or there are no courses that fit the definition of Mathematics or Computer Science return an empty result.

                # Output
                Before outputting, verify:
                    - Did I check if the image represents any kind of student transcript?
                    ...
                    - Did I use `None` for every credit and grade cell that I could not find a value for in the image?
